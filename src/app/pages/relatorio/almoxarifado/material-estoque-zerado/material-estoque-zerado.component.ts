...
@Component({
  selector: 'app-relatorio-material-estoque-zerado',
  templateUrl: './material-estoque-zerado.component.html',
  styleUrls: ['../almoxarifado.component.scss'],
})
export class RelatorioMaterialEstoqueZeradoComponent implements OnInit {
  constructor(
  ...
  ngOnInit(): void {
    console.log('Relatorio de Material Estoque Zerado');
  ...
  filtrar() {
    ...
    this.relatorioAlmoxarifado.MaterialEstoqueZerado(relatorioMaterialEstoque).subscribe({
      next: (res) => {
        this.baseService.relatorioMensagemModoImpressao(res, 'RelatorioMaterialEstoqueZerado');
      },
      error: (error) => {
