...
@Component({
  selector: 'app-balancete-estoque-lote-data-validade',
  templateUrl: './balancete-estoque-lote-data-validade.component.html',
})
export class RelatorioBalanceteEstoquelotedatavalidadeComponent implements OnInit {
  constructor(
  ...
  filtrar() {
    ...
    if (this.dataInicialSelecionada && this.dataFinalSelecionada) {
      ...
      this.relatorioAlmoxarifado.BalanceteEstoqueLoteDatavalidade(relatorioBalanceteEstoque).subscribe({
        next: (res) => {
          this.baseService.relatorioMensagemModoImpressao(res, 'RelatorioBalanceteEstoqueLoteDatavalidade');
        },
        error: (error) => {
