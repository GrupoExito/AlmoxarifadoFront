...
import { RelatorioMovimentacaoSaidaPorItem } from '@pages/relatorio/_models/relatorio-saida-material.model';
...
@Component({
  selector: 'app-relatorio-movimentacao-por-saida-item',
  templateUrl: './movimentacao-por-saida-item.component.html',
  styleUrls: ['../almoxarifado.component.scss'],
})
export class RelatorioMovimentacaoPorSaidaItemComponent implements OnInit {
  constructor(
  ...
  filtrar() {
    Swal.showLoading();
    const relatorioMovimentacaoSaida: RelatorioMovimentacaoSaidaPorItem = {
    ...
    this.relatorioAlmoxarifado.movimentacaoSaidaMaterialPorTipoItem(relatorioMovimentacaoSaida).subscribe({
      next: (res) => {
        this.baseService.relatorioMensagemModoImpressao(res, 'RelatorioMaterialEstoquePorItem');
      },
      error: (error) => {
