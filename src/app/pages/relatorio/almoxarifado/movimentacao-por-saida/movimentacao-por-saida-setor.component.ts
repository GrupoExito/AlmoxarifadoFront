...
@Component({
  selector: 'app-relatorio-movimentacao-por-saida-setor',
  templateUrl: './movimentacao-por-saida-setor.component.html',
  styleUrls: ['../almoxarifado.component.scss'],
})
export class RelatorioMovimentacaoPorSaidaSetorComponent implements OnInit {
  constructor(
  ...
  ngOnInit(): void {
    console.log('Relatorio de Movimentação por setor');
  ...
  filtrar() {
    ...
    this.relatorioAlmoxarifado.movimentacaoSaidaMaterialSetor(relatorioMovimentacaoSaida).subscribe({
      next: (res) => {
        this.baseService.relatorioMensagemModoImpressao(res, 'RelatorioMovimentacaoSaidaPorSetor');
      },
      error: (error) => {
