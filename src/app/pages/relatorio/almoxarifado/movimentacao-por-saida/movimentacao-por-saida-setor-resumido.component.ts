...
@Component({
  selector: 'app-relatorio-movimentacao-por-saida-setor-resumido',
  templateUrl: './movimentacao-por-saida-setor-resumido.component.html',
  styleUrls: ['../almoxarifado.component.scss'],
})
  export class RelatorioMovimentacaoPorSaidaSetorResumidoComponent implements OnInit {
  constructor(
  ...
  filtrar() {
    ...
    this.relatorioAlmoxarifado.movimentacaoSaidaMaterialSetorResumido(relatorioMovimentacaoSaida).subscribe({
      next: (res) => {
        this.baseService.relatorioMensagemModoImpressao(res, 'RelatorioMovimentacaoSaidaPorSetorResumido');
      },
      error: (error) => {
