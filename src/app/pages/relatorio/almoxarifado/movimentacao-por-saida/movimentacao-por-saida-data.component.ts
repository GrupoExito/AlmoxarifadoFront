...
@Component({
  selector: 'app-relatorio-movimentacao-por-saida-data',
  templateUrl: './movimentacao-por-saida-data.component.html',
  styleUrls: ['../almoxarifado.component.scss'],
})
export class RelatorioMovimentacaoPorSaidaDataComponent implements OnInit {
  constructor(
  ...
  filtrar() {
    ...
    this.relatorioAlmoxarifado.movimentacaoSaidaMaterialData(relatorioMovimentacaoSaida).subscribe({
      next: (res) => {
        this.baseService.relatorioMensagemModoImpressao(res, 'RelatorioMovimentacaoSaidaPorData');
      },
      error: (error) => {
