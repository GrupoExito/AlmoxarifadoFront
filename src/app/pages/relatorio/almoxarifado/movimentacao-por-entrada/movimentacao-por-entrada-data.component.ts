...
@Component({
  selector: 'app-relatorio-movimentacao-por-entrada-data',
  templateUrl: './movimentacao-por-entrada-data.component.html',
  styleUrls: ['../almoxarifado.component.scss'],
})
export class RelatorioMovimentacaoPorEntradaDataComponent implements OnInit {
  constructor(
  ...
  ngOnInit(): void {
    console.log('Relatorio de Movimentação por data');
  ...
  filtrar() {
    ...
    this.relatorioAlmoxarifado.movimentacaoEntradaMaterialData(relatorioMovimentacaoEntrada).subscribe({
      next: (res) => {
        this.baseService.relatorioMensagemModoImpressao(res, 'RelatorioMovimentacaoEntradaPorData');
      },
      error: (error) => {
