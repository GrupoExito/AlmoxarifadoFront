...
@Component({
  selector: 'app-relatorio-material-estoque-lote-data-validade',
  templateUrl: './material-estoque-lote-data-validade.component.html',
  styleUrls: ['../almoxarifado.component.scss'],
})
export class RelatorioMaterialEstoqueLoteDataValidadeComponent implements OnInit {
  constructor(
  ...
  ngOnInit(): void {
    console.log('Relatorio de Material Estoque lote/Data validade');
  ...
  filtrar() {
    ...
    this.relatorioAlmoxarifado.MaterialEstoqueLoteDataValidade(relatorioMaterialEstoque).subscribe({
      next: (res) => {
        this.baseService.relatorioMensagemModoImpressao(res, 'RelatorioMaterialEstoqueLoteDataValidade');
      },
      error: (error) => {
